...
class RAGSystem:
    ...
    def process_urls(self, urls):
        ...
        try:
            ...
            if not valid_urls:
                return "Error: No valid URLs found. Please ensure URLs start with http:// or https://"
            
            st.info(f"Processing {len(valid_urls)} URLs: {', '.join(valid_urls)}")
            ...
            if not data or len(data) == 0:
                return "Error: No content could be extracted from the provided URLs. Please check if the URLs are accessible."
            
            st.info(f"Successfully loaded {len(data)} documents from URLs")
            
            # Also store the raw text for potential summarization
            combined_text = "\n\n".join([doc.page_content for doc in data if hasattr(doc, 'page_content') and doc.page_content])
            
            if not combined_text.strip():
                return "Error: No text content could be extracted from the URLs"
            ...
            if not splits or len(splits) == 0:
                return "Error: Could not create text chunks from the URL content"
            ...
            self.source_name = ", ".join(valid_urls)
            st.session_state.source_name = ", ".join(valid_urls)
            ...
            return f"URLs processed successfully! Processed {len(valid_urls)} URLs with {len(splits)} text chunks."
...
def main():
    ...
    st.markdown("Upload content (PDF, URLs, YouTube) for QA and summarization using OpenAI GPT")
    ...
    with st.expander("📚 Application Documentation"):
        ...
        ### Overview
        This application combines Retrieval-Augmented Generation (RAG) with summarization capabilities for PDFs, websites, and YouTube videos using OpenAI's GPT models.
